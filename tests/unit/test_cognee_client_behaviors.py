...
async def test_add_uses_multipart_not_json(client):
    ...
    with patch.object(client._client, "request", new_callable=AsyncMock, return_value=mock_response) as mock_req:
        ...
        # 应传 files= 参数（multipart），而非 json=
        assert kwargs.get("files") is not None, "add() 必须使用 multipart files= 参数"
        assert kwargs.get("json") is None, "add() 不能使用 json= 参数"


@pytest.mark.asyncio
async def test_add_field_name_is_datasetName(client):
    """add() data= 字段名必须是 datasetName（非 dataset）"""
    mock_response = httpx.Response(200, json={"id": "data_1"})
    with patch.object(client._client, "request", new_callable=AsyncMock, return_value=mock_response) as mock_req:
...
async def test_add_encodes_data_as_bytes(client):
    ...
    with patch.object(client._client, "request", new_callable=AsyncMock, return_value=mock_response) as mock_req:
        ...
        assert content == "中文内容".encode("utf-8")
...
async def test_search_passes_search_type(client):
    ...
    with patch.object(client._client, "request", new_callable=AsyncMock, return_value=mock_response) as mock_req:
        ...
        _, kwargs = mock_req.call_args
        assert kwargs["json"]["search_type"] == "CHUNKS"
...
async def test_search_default_search_type_is_graph_completion(client):
    ...
    with patch.object(client._client, "request", new_callable=AsyncMock, return_value=mock_response) as mock_req:
        ...
        _, kwargs = mock_req.call_args
        assert kwargs["json"]["search_type"] == "GRAPH_COMPLETION"
        ...
        json=[
            {
                ...
                "text": "Cognee 是知识图谱引擎",
                "score": 0.9,
                "node_type": "entity",       # 未知字段
                "layer": "summarization",     # 未知字段
            }
        ],
    )
    with patch.object(client._client, "request", new_callable=AsyncMock, return_value=mock_response):
...
async def test_search_with_all_search_types(client):
    ...
    for search_type in valid_types:
        with patch.object(client._client, "request", new_callable=AsyncMock, return_value=mock_response) as mock_req:
            ...
            assert payload["search_type"] == search_type
...
async def test_cognify_without_datasets_omits_key(client):
    ...
    mock_response = httpx.Response(200, json={"run_id": "r1", "status": "pending"})
    with patch.object(client._client, "request", new_callable=AsyncMock, return_value=mock_response) as mock_req:
...
async def test_health_check_uses_health_path(client):
    ...
    with patch.object(client._client, "request", new_callable=AsyncMock, return_value=mock_response) as mock_req:
        ...
        assert "/api/v1" not in kwargs["url"]
