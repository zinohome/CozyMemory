...
import { operatorApi, type EngineStatus, type KnowledgeDataset } from "@/lib/api";
...
function UsersTable({ userIds }: { userIds: string[] }) {
  ...
  const memQueries = useQueries({
    queries: shown.map((uid) => ({
      queryKey: ["operator", "mem-count", uid],
      queryFn: () => operatorApi.listConversations(uid).then((r) => r.total ?? r.data?.length ?? 0),
      staleTime: 60_000,
    })),
  });
...
export default function DashboardPage() {
  ...
  function handleRefresh() {
    qc.invalidateQueries({ queryKey: ["operator", "health"] });
    qc.invalidateQueries({ queryKey: ["operator", "users"] });
    qc.invalidateQueries({ queryKey: ["operator", "datasets"] });
    qc.invalidateQueries({ queryKey: ["operator", "mem-count"] }); // invalidates all mem-count/* entries
