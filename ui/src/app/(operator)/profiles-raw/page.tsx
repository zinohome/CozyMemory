...
import { operatorApi, type ProfileItem, type ProfileResponse } from "@/lib/api";
...
export default function ProfilesPage() {
  ...
  const contextQuery = useQuery({
    queryKey: ["operator", "profile-context", userId],
    queryFn: () => operatorApi.getProfileContext(userId),
    enabled: !!userId,
  });

  const deleteMutation = useMutation({
    mutationFn: (profileId: string) => operatorApi.deleteProfileItem(userId, profileId),
    onMutate: async (profileId) => {
      await qc.cancelQueries({ queryKey: ["operator", "profile", userId] });
      const previous = qc.getQueryData<ProfileResponse>(["operator", "profile", userId]);
      qc.setQueryData<ProfileResponse>(["operator", "profile", userId], (old) => {
    ...
    onError: (e, _id, ctx) => {
      if (ctx?.previous) qc.setQueryData(["operator", "profile", userId], ctx.previous);
      toast.error((e as Error).message);
    },
    onSuccess: () => toast.success(t("profiles.delete.success")),
    onSettled: () => qc.invalidateQueries({ queryKey: ["operator", "profile", userId] }),
  });

  const addMutation = useMutation({
    mutationFn: () => operatorApi.addProfileItem(userId, newItem),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["operator", "profile", userId] });
