...
import { operatorApi, type ConversationListResponse, type ConversationMemory } from "@/lib/api";
...
export default function MemoryLabPage() {
  ...
  const searchMutation = useMutation({
    mutationFn: () =>
      operatorApi.searchConversations({
        query: searchQuery,
        user_id: userId,
        limit: 10,
        memory_scope: "long",
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: string) => operatorApi.deleteConversation(id),
    // Optimistic delete: 立即从 cache 移除，失败则回滚
    onMutate: async (id) => {
      await qc.cancelQueries({ queryKey: ["operator", "memories", userId] });
      const previous = qc.getQueryData<ConversationListResponse>(["operator", "memories", userId]);
      qc.setQueryData<ConversationListResponse>(["operator", "memories", userId], (old) => {
        if (!old) return old;
    ...
    onError: (e, _id, ctx) => {
      if (ctx?.previous) qc.setQueryData(["operator", "memories", userId], ctx.previous);
      toast.error((e as Error).message);
    },
    onSuccess: () => toast.success(t("memory.delete.success")),
    onSettled: () => qc.invalidateQueries({ queryKey: ["operator", "memories", userId] }),
  });
